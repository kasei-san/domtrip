...
補足: コミットの author 名・メールは push 時に公開メタデータとして残る。公開用の git identity を使うこと。新規ファイル・スクリプトを作ったら、コミット前に `grep -nE '/Users/|@' <file>` 等で上記に該当する文字列が無いか確認する。
...
- 属性 `flagged`（種類ではない）: details 内に文字列 `正解要確認` があると立つ（アプリで注意表示）
...
4. 間違えたら `誤答ログ.md` に**論点単位＋ミス種別**で追記し、間隔反復（翌日→3日後→1週間後…）の次回復習日を書く。
...
## 科目3 図表問題と著作権（重要な方針）
- **2026-09-21 方針変更**: テキストの選択肢原文（問題文＋ア〜エの文言）は逐語転記してよい。地名・施設名・郷土料理名などの列挙型選択肢（組合せ問題、国立公園内の観光地選択、神社・橋・城・半島・滝・諸島などの名称当て問題等）はこれに該当し、`- ア. 本文` 形式で選択肢を収録し、mcq カードとしてビルドする。
- **変更なし**: 科目3の図表依存設問（運賃計算図・地図・時刻表・料金表など、本当に図表がないと問題文自体が成立しない設問）は本文を作り込まず「【図表は過去問PDF参照】＋正解＋解説」に留める（図表は再現・捏造しない）。これらは引き続き「解答レビュー」形式（review、選択肢なし・自己採点）のまま。
- **過去問の図表を画像として公開リポジトリに置かない**（試験問題の再配布懸念）。図を扱うなら ①チャットでユーザーが貼った画像を読んで解説する ②どうしても埋め込むなら `.gitignore` 済みのローカル専用フォルダに限定。
- 1つの `###` ブロックに①②③④のような複数の小問（各々に独自のア〜エ選択肢がある）が含まれる場合、本アプリのカード形式（1カード=1組のア〜エ）では正しく表現できないため、選択肢文はテキストとして本文に収録しつつ mcq 化はせず review のまま維持する。
