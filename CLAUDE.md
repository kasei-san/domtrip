...
これはソフトウェアのコードベースではなく、**国内旅行業務取扱管理者試験**（domtrip = domestic trip）の過去問PDFを集めた学習用ディレクトリである。ビルド・lint・テストの仕組みは存在しない。git 管理もされていない。

想定される作業は、PDF の中身を読み取って問題を解説する・解答と照合する・出題傾向を分析するといった**学習支援タスク**である。PDF の読み取りには Read ツール（`pages` 指定）を使う。
